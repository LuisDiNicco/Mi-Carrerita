...
export const HistoryTable = () => {
  ...
  const [status, setStatus] = useState(SubjectStatus.APROBADA);
  ...
  const handleCreateRecord = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/academic-career/subjects/${subjectId}`, {
        method: 'PATCH',
        headers: {
      ...
      const graphResponse = await fetch(`${API_URL}/academic-career/graph`);
      if (graphResponse.ok) {
