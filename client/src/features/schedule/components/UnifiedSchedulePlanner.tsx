...
import { MousePointerClick, Edit, CheckCircle } from 'lucide-react';
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex justify-center pt-2">
                <div className="flex items-center gap-2 text-[11px] bg-surface border border-app px-4 py-2 text-muted shadow-sm md:rounded-full rounded-md text-center max-w-full">
                    {mode === 'AVAILABILITY' ? (
                        <><MousePointerClick size={14} className="text-unlam-500" /> Pulsa o arrastra para marcar disponibilidad.</>
                    ) : mode === 'OPTIONS' ? (
                        <><Edit size={14} className="text-primary" /> En esta etapa puedes ver opciones solapadas de oferta y elegir una por celda.</>
                    ) : (
                        <><CheckCircle size={14} className="text-emerald-500" /> Esta es tu cursada final elegida. Puedes volver al paso 2 para ajustar.</>
                    )}
                </div>
            </div>
        </div>
