...
Este documento expone las justificaciones de arquitectura subyacente, analizando los compromisos (trade-offs), las decisiones de diseño adoptadas e insights fundamentales de cómo opera internamente Mi-Carrerita. Está orientado a lñderes técnicos e ingenieros de software revisando la robustez y escalabilidad.
...
- **Backend (NestJS + Prisma):** Se utiliza una arquitectura interna **Modular / Features based**. A diferencia del MVC de la vieja escuela acoplado fuertemente por Capas, el dominio se sectoriza por caracterñsticas de negocio (ej. `academic-career`, `history`, `trophies`).
...
## 2. Decisiones y Trade-offs Crñticos de Frontend
...
- **Decisión:** Zustand maneja un state global pero expone variables como hooks autónomos, por tanto si el array de materias muta, solo el componente especñfico escuchándolo se actualiza, eludiendo el overhead del Virtual DOM.
...
- **Arquitectura Moderna (Event-Driven):** 
  ...
  2. Emite el evento asñncrono `Subject.Updated`.
...
- **Trade-off:** Es de *Consistency Eventual*. Tras aprobar una materia particular, el trofeo se actualizarña luego de unos milisegundos tras la respuesta 200 de Node, por lo que el Frontend no obtendrá un payload de trofeos en esa misma response, derivando en tener que usar Polling / WebSocket si se quisiera alerta real-time.
...
1. **Verificador Topological (Materias Clave):** Realiza un BFS transversal validado hasta hojas en grafos dependientes para inferir qué nodo superior destraba más ramas. Es asñncrono y memoizado si el layout curricular plan no muta.
...
- **Múltiples Carreras Transversales:** Actualmente el diseño Data-Model está altamente acoplado a *un user con una carrera*. Cambiar esto impactarña el Schema relacional requiriendo un `EnrollmentModel`.
- **Caché Distribuido (Redis):** Si la consulta de los recorridos se estresa y el árbol general del plan 2023 es estático, el `plan graph` completo deberña memoizarse en un cluster de Redis y no servirse mediante Prisma en cada F5 de la SPA, ahorrando ciclos puros de I/O contra la RDBMS de producción.
